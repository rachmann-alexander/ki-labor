#!/usr/bin/env python3
# file: make_plot_06_edp_vs_throughput_grid_swapped.py
# Grid-SVG: 06_throughput_vs_edp_per_image_facet.svg (rows=model, cols=precision)
# x = EDP per image [J·s], y = Throughput [img/s]
...
def _ensure_edp(df: pd.DataFrame) -> pd.DataFrame:
    # energy_per_img_J ableiten, falls nötig
...
def make_facet_plot(df, out_dir, xlog=False, ylog=False, vary_x=False):
    ...
    # X-Limits (global oder variabel), Y-Limits global
    if not vary_x:
        xlim=_limits_from(d["edp_image_Js"], log=xlog, pad_frac=0.06)
        if xlim is None: return
    else:
        xlim=None
    ylim=_limits_from(d["throughput_img_s"], log=ylog, pad_frac=0.10)
        ...
        for c,pz in enumerate(precisions):
            ...
            for prof,gprof in sub.groupby("profile"):
                ...
                for bval,gb in gprof.groupby("batch"):
                    ...
                    ax.scatter(gb["edp_image_Js"], gb["throughput_img_s"],
                               s=POINT_SIZE, marker=marker, color=color,
                               alpha=0.9, edgecolors="none", linewidths=0)

            if r==nrows-1: ax.set_xlabel("EDP per image [J·s]")
            ...
            if vary_x:
                xloc=_limits_from(sub["edp_image_Js"], log=xlog, pad_frac=0.06)
    ...
    # Legenden (rechts) konsistent zu deinen „passt“-Werten
    ...
    fig.suptitle("Throughput vs. EDP (per image) — faceted by model × precision", y=0.98, fontsize=11)

    os.makedirs(out_dir, exist_ok=True)
    out_path=os.path.join(out_dir,"06_throughput_vs_edp_per_image_facet.svg")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    print("✔︎ SVG:", out_path)

def main():
    ap=argparse.ArgumentParser(description="Grid-Plot: Throughput vs EDP per image (SVG)")
    ...
    ap.add_argument("--vary-x", action="store_true", help="X-Achse (EDP) pro Panel variieren")
    args=ap.parse_args()
