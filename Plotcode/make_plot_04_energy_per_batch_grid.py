#!/usr/bin/env python3
# file: make_plot_04_energy_per_batch_grid.py
# Grid-SVG: 04_energy_per_batch_vs_batch_facet.svg (rows=model, cols=precision)
# x = Batch size, y = Energy per batch [J/Batch]
...
def _ensure_energy_per_batch(df: pd.DataFrame) -> pd.DataFrame:
    """
    Stellt y=energy_per_batch_J bereit.
    Priorität:
      1) falls 'energy_per_inference_J' existiert (historisch: J/Batch)
      2) sonst: energy_per_img_J * batch
      3) sonst: (avg_power_W / throughput_img_s) * batch
    """
    if "energy_per_inference_J" in df.columns and not df["energy_per_inference_J"].isna().all():
        df["energy_per_batch_J"] = pd.to_numeric(df["energy_per_inference_J"], errors="coerce")
        return df

    batch = pd.to_numeric(df.get("batch", np.nan), errors="coerce")
    if "energy_per_img_J" in df.columns:
        epi = pd.to_numeric(df["energy_per_img_J"], errors="coerce") * batch
    else:
        pw = pd.to_numeric(df.get("avg_power_W", np.nan), errors="coerce")
        th = pd.to_numeric(df.get("throughput_img_s", np.nan), errors="coerce")
        epi = np.where((pw > 0) & (th > 0), (pw / th) * batch, np.nan)
    df["energy_per_batch_J"] = epi
    return df
...
def make_facet_plot(df: pd.DataFrame, out_dir: str,
    ...
    df = _ensure_energy_per_batch(df.copy())
    d  = df.dropna(subset=["model","precision","profile","batch","energy_per_batch_J"])
    if d.empty:
    ...
    # X-Limits (diskret, kleiner Rand)
    xvals = pd.to_numeric(d["batch"], errors="coerce")
    xlim = (float(xvals.min()) - 4.0, float(xvals.max()) + 4.0)

    # Globale Y-Limits (nur wenn NOT vary_y)
    if not vary_y:
        ylim_global = _limits_from(d["energy_per_batch_J"], log=ylog, pad_frac=0.10)
    ...
    for r, m in enumerate(models):
        for c, pz in enumerate(precisions):
            ...
            ax.set_xlim(*xlim)
            if vary_y:
                ylim = _limits_from(sub["energy_per_batch_J"], log=ylog, pad_frac=0.10)
    ...
    # Legenden rechts – etwas weiter nach rechts verschoben
    ...
    fig.suptitle("Energy per Batch vs. Batch Size — faceted by model × precision", y=0.98, fontsize=11)

    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "04_energy_per_batch_vs_batch_facet.svg")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    print("✔︎ SVG geschrieben:", out_path)

def main():
    ap = argparse.ArgumentParser(description="Grid-Plot: Energy per Batch vs Batch Size (SVG)")
