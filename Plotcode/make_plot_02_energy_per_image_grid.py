#!/usr/bin/env python3
# file: make_plot_02_energy_per_image_grid.py
# Grid-SVG: 02_energy_per_image_vs_batch_facet.svg (rows=model, cols=precision)
# y = Energy per Image [mJ]  (J werden formatiert zu mJ)
# - Farbe = profile (slow/medium/fast, farbiger Text in Legende)
# - Marker = batch (32:o, 64:s, 128:^)
# - Achsen: global (Default) oder je Panel variabel (--vary-x/--vary-y)
...
def _ensure_energy_per_img(df: pd.DataFrame) -> pd.DataFrame:
    if "energy_per_img_J" not in df.columns or df["energy_per_img_J"].isna().all():
        if {"avg_power_W","throughput_img_s"}.issubset(df.columns):
            df["energy_per_img_J"] = np.where(
                (pd.to_numeric(df["avg_power_W"], errors="coerce")>0) &
                (pd.to_numeric(df["throughput_img_s"], errors="coerce")>0),
                pd.to_numeric(df["avg_power_W"], errors="coerce") /
                pd.to_numeric(df["throughput_img_s"], errors="coerce"),
                np.nan
            )
    return df
...
def make_facet_plot(df: pd.DataFrame, out_dir: str,
                    ...
                    vary_x: bool=False, vary_y: bool=False):
    need = ["model","precision","profile","batch","energy_per_img_J"]
    for c in need:
        if c not in df.columns:
            if c == "energy_per_img_J":
                continue
            raise ValueError(f"Spalte fehlt im Manifest: {c}")

    df = _ensure_energy_per_img(df.copy())
    d = df.dropna(subset=["model","precision","profile","batch","energy_per_img_J"])
    if d.empty:
    ...
    # X = batch (diskret)
    x_all = pd.to_numeric(d["batch"], errors="coerce")
    xlim_global = None if vary_x else _limits_from(x_all, log=xlog, pad_frac=0.05)
    ylim_global = None if vary_y else _limits_from(d["energy_per_img_J"], log=ylog, pad_frac=0.10)
    if (not vary_x and xlim_global is None) or (not vary_y and ylim_global is None):
        print("[i] Konnte globale Limits nicht bestimmen.")
        return
    ...
    for r, m in enumerate(models):
        for c, pz in enumerate(precisions):
            ...
            if c == 0:         ax.set_ylabel("Energy per image [mJ]")
            ax.set_title(f"{m} — {pz}")

            # X-Ticks = vorhandene Batches
            ...
            ax.yaxis.set_major_formatter(FuncFormatter(fmt_mJ))
            ...
            if vary_x:
                xlim = _limits_from(sub["batch"], log=xlog, pad_frac=0.05)
                if xlim: ax.set_xlim(*xlim)
            else:
                ax.set_xlim(*xlim_global)

            if vary_y:
                ylim = _limits_from(sub["energy_per_img_J"], log=ylog, pad_frac=0.10)
    ...
    # Legenden rechts – näher an die Plots gerückt, aber außerhalb
    fig.subplots_adjust(right=0.86, top=0.90, bottom=0.12, wspace=0.28, hspace=0.32)

    # Profile: farbiger Text
    ...
    leg_prof = fig.legend(handles=prof_handles, title="profile",
                          loc="center right", bbox_to_anchor=(0.965, 0.62), frameon=False)
    ...
    fig.legend(handles=batch_handles, title="batch size",
               loc="center right", bbox_to_anchor=(0.965, 0.30), frameon=False)

    fig.suptitle("Energy per Image vs. Batch Size — faceted by model × precision", y=0.98, fontsize=11)

    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "02_energy_per_image_vs_batch_facet.svg")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close(fig)
    print("✔︎ SVG geschrieben:", out_path)

def main():
    ap = argparse.ArgumentParser(description="Grid-Plot: Energy per Image vs Batch (SVG)")
    ...
    ap.add_argument("--vary-x", action="store_true", help="X-Achse pro Panel variieren")
    ...
    make_facet_plot(df, args.out, xlog=args.xlog, ylog=args.ylog,
                    vary_x=args.vary_x, vary_y=args.vary_y)
